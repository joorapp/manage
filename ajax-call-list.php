...
$sel = mysqli_query($conn, "SELECT COUNT(*) AS allcount FROM {$prefix}enquiries
    ...
    INNER JOIN {$prefix}statuses ON {$prefix}statuses.id = {$prefix}enquiries.status_id WHERE {$prefix}enquiries.status_id = 1 AND 1 ".$roleCondition);
...
$sel = mysqli_query($conn, "SELECT COUNT(*) AS allcount FROM {$prefix}enquiries 
    ...
    WHERE {$prefix}enquiries.status_id = 1 AND 1 ".$roleCondition." ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "SELECT *, aby_users.name AS aby, ato_users.name AS ato, {$prefix}enquiries.id as ed_no, {$prefix}enquiries.mobile as enq_mobile, {$prefix}products.id as p_id FROM {$prefix}enquiries
    INNER JOIN {$prefix}products ON {$prefix}products.id = {$prefix}enquiries.product_id 
    INNER JOIN {$prefix}statuses ON {$prefix}statuses.id = {$prefix}enquiries.status_id 
    LEFT JOIN {$prefix}users AS aby_users ON aby_users.id = {$prefix}enquiries.user_id 
    LEFT JOIN {$prefix}users AS ato_users ON ato_users.id = {$prefix}enquiries.assign_to 
