...
$sel = mysqli_query($conn, "SELECT COUNT(*) AS allcount FROM {$prefix}enquiries
    ...
    INNER JOIN {$prefix}statuses ON {$prefix}statuses.id = {$prefix}enquiries.status_id WHERE {$prefix}enquiries.status_id = 6 AND 1 ".$roleCondition);
...
$sel = mysqli_query($conn, "SELECT COUNT(*) AS allcount FROM {$prefix}enquiries 
    ...
    WHERE {$prefix}enquiries.status_id = 6 AND 1 ".$roleCondition." ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
