...
$sel = mysqli_query($conn,"select count(*) as allcount from {$prefix}customers where  pro_id = 6 and delete_status = 0  and 1");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of records with filtering
$sel = mysqli_query($conn,"select count(*) as allcount from {$prefix}customers  where  pro_id = 6 and delete_status = 0 and  1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$empQuery = "select *,  {$prefix}customers.id as ed_no from {$prefix}customers  where  pro_id = 6 and delete_status = 0  and 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
